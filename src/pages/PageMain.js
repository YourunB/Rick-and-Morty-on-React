...
import {
  updateSaveCharacters,
  upadteScrollCharacters,
  updatePage,
  updateLoad,
...
export const PageMain = () => {
  ...
  function scrollLoad() {
    ...
    if (
      position >= threshold &&
      dataRedux.page < 42 &&
      dataRedux.load === false &&
      pagination === false
    ) {
      dispatch(updatePage(dataRedux.page + 1));
      dispatch(updateLoad(true));
      getData(
        `https://rickandmortyapi.com/api/character/?page=${dataRedux.page + 1}`,
        "scroll",
      );
    }
  }
  ...
  function locationNav() {
    if (
      ...
      window.location.search !== `?page=${dataRedux.page}`
    ) {
      ...
      if (searchText === "?page=" && !isNaN(searchPage)) {
        if (Number(searchPage) !== dataRedux.page && Number(searchPage) > 0) {
          ...
          setTimeout(() => {
            getData(
              `https://rickandmortyapi.com/api/character/?page=${Number(searchPage)}`,
            );
          }, 0);
        }
      }
  ...
  return (
    <main className="PageMain">
      ...
      <section className="PageMain__head">
        ...
        <div>
          <input
            className="PageMain__head__input"
          ...
          ></input>
          <label className="PageMain__head__label" htmlFor="select-pagination">
            Pagination:
          </label>
        </div>
      </section>
        ...
        {pagination === true ? (
          <div className="PageMain__cards__controls">
            <NavLink to={`/main?page=${dataRedux.page - 1}`}>
              <button
                className="PageMain__btn"
                disabled={dataRedux.page === 1 ? true : false}
                onClick={() => {
                  prevPage();
                }}
              >
            ...
            <NavLink to={`/main?page=${dataRedux.page + 1}`}>
              <button
                className="PageMain__btn"
                disabled={dataRedux.page === 42 ? true : false}
                onClick={() => {
                  nextPage();
                }}
              >
      ...
      <img
        ref={animLoad}
        className="PageMain__load PageMain__load_hide"
        src="./images/load.gif"
        alt="Load"
      />
      <button
